...
This guide explains how the **Suspicious Email Triage** API tells orchestrators (Docker, Kubernetes) whether a container is **alive** and whether it can **serve traffic**. It implements [TBD §1.2](TBD.md#12-health-checks-and-uptime-p0) — the free, local path without paid uptime vendors.

**Related:** [architecture.md](architecture.md), [infra/docker/docker-compose.yml](../infra/docker/docker-compose.yml), [deploy/helm/triage](../deploy/helm/triage).
...
**Not in scope for this free path:** external uptime SaaS, `@godaddy/terminus` graceful shutdown (mentioned in TBD as optional future work). Counters and alerts live in [metrics_and_alerting_guide.md](metrics_and_alerting_guide.md).
