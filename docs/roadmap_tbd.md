...
- **`#flow` Live flow dashboard** — **single-viewport** SOC wall (`flow-dashboard--viewport`); compact 3×3 gauge grid + **wider right meta column** for clocks/sim/stats; single UTC analog clock (no toolbar duplicate); configurable auto-refresh (0.5 s–30 s); **FlowWarningBadge** SVG indicator (theme-safe gray → amber/red when threshold breached); volatility jitter gauge
...
- **Outbound verdict webhooks** — per mail platform default URL in Postgres **`ingest_clients`**; self-registration `PUT /ingest/register/:clientId` and Go `PUT /v1/clients/:id` with `X-Ingest-Registration-Token`; JWT UI form + `PUT /ingest/clients/:id` (`ingest.clients.write`); optional per-message `callbackUrl`; POST JSON + HMAC when analysis completes; polling `GET /ingest/internal/verdict/:id`; optional Kafka `email.review.completed`; dev **mock-verdict-callback** (:4569) — [data_guide_verdict_webhooks.md](data_guide_verdict_webhooks.md).
