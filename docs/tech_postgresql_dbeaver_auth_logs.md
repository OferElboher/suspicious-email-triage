...
Prerequisites for database connectivity: [tech_postgresql_dbeaver_windows.md](tech_postgresql_dbeaver_windows.md) and [stack_guide_windows_docker_databases.md](stack_guide_windows_docker_databases.md).

Bootstrap admin email, password, and recovery: [auth_guide_dev_admin_credentials.md](auth_guide_dev_admin_credentials.md).

**Login fails but `auth_users` shows your email?** Password is bcrypt-hashed and not visible in DBeaver — see [auth_guide_dev_auth_recovery.md](auth_guide_dev_auth_recovery.md).
...
The **`admin` role** has **all permissions**: every UI tab (triage, analytics, dev simulation when in `dev`), Django user administration, and every protected HTTP route (`/reviews`, `/metrics`, `/logs/search`, `/dev/*`, etc.). User CRUD is in Django admin — see [auth_guide_django_admin_users.md](auth_guide_django_admin_users.md).
...
Replace `you@example.com` with an address you control. Full detail: [auth_guide_dev_admin_credentials.md](auth_guide_dev_admin_credentials.md).
...
See [auth_guide_dev_admin_credentials.md](auth_guide_dev_admin_credentials.md) for change/recover flows.
...
Use **Forgot password** on the sign-in screen. Step-by-step: [auth_guide_dev_admin_credentials.md](auth_guide_dev_admin_credentials.md).

#### Create additional admins (after first login)

Sign in as admin → **User administration** → create user → assign role **`admin`**. See [auth_guide_django_admin_users.md](auth_guide_django_admin_users.md).
...
Full role/permission reference: [auth_guide_rbac.md](auth_guide_rbac.md).
...
| Email in `auth_users`, login fails | [auth_guide_dev_auth_recovery.md](auth_guide_dev_auth_recovery.md) |
| `curl` connection reset | API down — fix Mongo ([Step 1b](#step-1b--mongodb-stuck-or-backend-fatal-startup)), not an auth user issue |
| Connected to wrong Postgres | Port `5432` conflict — see [tech_postgresql_dbeaver_windows.md](tech_postgresql_dbeaver_windows.md) |
...
Best when you already have an admin JWT. See [auth_guide_rbac.md](auth_guide_rbac.md) for login.

```bash
# 1) Sign in (replace email/password — see auth_guide_dev_admin_credentials.md)
...
| Full-access login | Your email / `temp-admin-pswd` ([Step 3](#step-3--create-the-bootstrap-admin-user), [auth_guide_dev_admin_credentials.md](auth_guide_dev_admin_credentials.md)) |
| Reset auth + new admin | [auth_guide_dev_auth_recovery.md](auth_guide_dev_auth_recovery.md) |
...
- [auth_guide_rbac.md](auth_guide_rbac.md) — login, admin role, `logs.read`
- [tech_postgresql_dbeaver_windows.md](tech_postgresql_dbeaver_windows.md) — first-time DBeaver connection
- [stack_guide_windows_docker_databases.md](stack_guide_windows_docker_databases.md) — start Postgres and verify ports
