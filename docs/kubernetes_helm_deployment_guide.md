...
For broader context about what the product does and how its services interact, start with the [documentation index](README.md) — especially [architecture.md](architecture.md), [deployment.md](deployment.md), [env_configuration_guide.md](env_configuration_guide.md), and [production_setup_guide.md](production_setup_guide.md).
...
This follows the [twelve-factor app](https://12factor.net/config) pattern: **config in the environment**, not baked into container images. See also [env_configuration_guide.md](env_configuration_guide.md).
...
For production maturity, prefer **External Secrets Operator**, AWS Secrets Manager, Azure Key Vault, or HashiCorp Vault instead of plain Kubernetes Secrets checked into Helm values — see [TBD.md](TBD.md) section on secrets management.
...
The application flow (see [architecture.md](architecture.md)) is unchanged in Kubernetes; only **where** processes run changes.
...
\*The chart sets `NEO4J_ENABLED` and `NEO4J_PASSWORD` via ConfigMap/Secret. Values files include `neo4j.enabled` and storage sizing for future in-cluster Neo4j templates; today, run Neo4j via [neo4j_wsl_windows_setup_guide.md](neo4j_wsl_windows_setup_guide.md) locally or a managed service in cloud environments, and point connection settings through additional ConfigMap overrides or an extended values file.

When **in-cluster databases are disabled** (staging/prod), you must supply **remote connection strings** for MongoDB, PostgreSQL, Redis, and Kafka — the default ConfigMap still assumes in-cluster hostnames. Extend your environment values or pass `--set config.kafkaBrokers=...` and add custom keys to the ConfigMap template as your platform matures. See [deployment.md](deployment.md) for the expected variable names (`MONGO_URI`, `STATISTICS_PG_URL`, `CELERY_BROKER_URL`, etc.).
...
4. **Container images** built and available to the cluster:
   ...
   Build locally with [deployment.md](deployment.md) / `scripts/setup-and-build-dev.sh`, then push to a registry your cluster can pull from. Set `global.imagePullSecrets` if using a private registry.
...
Integrate External Secrets Operator or your cloud provider’s secret store so Pods receive rotated credentials without Helm ever storing plaintext in CI logs. This aligns with [production_setup_guide.md](production_setup_guide.md) and the P0 item in [TBD.md](TBD.md).
...
Production checklist (see also [production_setup_guide.md](production_setup_guide.md)):
...
`values.yaml` includes `nodeWorker.enabled: false` for an optional Node Kafka dispatcher. Enable and add a template when your deployment needs that path instead of or in addition to the Python dispatcher — see [worker-architecture.md](worker-architecture.md).

---

## Relationship to Docker Compose local dev

Most developers still use **Docker Compose** for day-to-day work (`infra/docker/docker-compose.yml`). That path is documented in [deployment.md](deployment.md), [full_stack_run_guide_mongo_db_node_worker_react.md](full_stack_run_guide_mongo_db_node_worker_react.md), and [windows_dev_startup_run_guide.md](windows_dev_startup_run_guide.md).
...
| [deployment.md](deployment.md) | Image build and Compose-based dev deployment |
| [env_configuration_guide.md](env_configuration_guide.md) | Environment variables and profile files |
| [production_setup_guide.md](production_setup_guide.md) | Hardening beyond “it runs” |
| [AUTHENTICATION_AND_RBAC.md](AUTHENTICATION_AND_RBAC.md) | JWT, roles, bootstrap admin |
| [worker-architecture.md](worker-architecture.md) | Celery vs Kafka worker paths |
| [neo4j_phishing_graph_guide.md](neo4j_phishing_graph_guide.md) | Graph database features |
| [TBD.md](TBD.md) | Production roadmap (secrets, observability, HA) |
| [pre_push_tests_and_stack_verification.md](pre_push_tests_and_stack_verification.md) | Tests before shipping chart changes |
